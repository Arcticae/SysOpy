...
void start_server(){
    ...
    server_attributes.mq_msgsize = sizeof(struct msgbuffer_new);
    server_attributes.mq_maxmsg = 10;

    server_queue= mq_open(SRV_DIRECTORY, O_RDONLY | O_CREAT | O_EXCL | O_NONBLOCK, S_IRWXU | S_IRWXG ,&server_attributes);
    if(server_queue==-1){
...
int main(int argc,char**argv){
    ...
    while(1){
        ...
        if(mq_receive(server_queue,(char*) &message,sizeof(struct msgbuffer_new),0)==-1 ){      //we do not wait for the communicate, because we have infinite loop
            if(errno!=EAGAIN) {
            ...
            do{
                ercode=mq_send(client_queues[message.index],(char*)&response,sizeof(struct msgbuffer_new),0);
